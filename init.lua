...
local function do_git_commit()
    ...
    -- 1. Check for a stale lock file and remove it to prevent "cooked" processes
    ie.os.execute("rm -f " .. world_path .. "/.git/index.lock")

    -- 2. Get commit count
    local count_raw = shell_exec("cd " .. minetest.get_worldpath() .. " && git rev-list --count HEAD 2>/dev/null || echo 0")
    local count = tonumber(count_raw) or 0
    
    -- 3. The Command
    -- We removed the "> /dev/null 2>&1" so you can see errors in your terminal
    local cmd = string.format(
        "cd %q && ( [ ! -d .git ] && git init && git add . && git commit -m '0' || true ); " ..
        "nice -n 19 ionice -c 3 git add . && nice -n 19 ionice -c 3 git commit -m '%d' &",
        world_path, count
    )
...
minetest.register_chatcommand("git", {
    ...
    func = function(name, param)
        ...
        elseif subcommand == "commit" then
            local id = do_git_commit()
            return true, "Attempted snapshot. ID should be: " .. id
